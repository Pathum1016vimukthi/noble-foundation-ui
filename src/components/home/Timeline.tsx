...
const Timeline = ({ items }: TimelineProps) => {
  ...
  return (
    <div className="max-w-4xl mx-auto mb-16" ref={containerRef}>
      <h3 className="text-2xl text-charcoal text-center mb-4">Competition Timeline</h3>
      <p className="text-center text-muted-foreground mb-10">Key dates for the 2026 season. Plan ahead and stay on track.</p>
      ...
      <div className="relative pl-6">
        ...
        {items.map((item, index) => {
          ...
          return (
            <div key={item.event} className="relative flex items-start gap-4 pb-6 last:pb-0 lg:items-start">
              <div
              ...
              <div
                ...
                className={`phase-card bg-card border border-border rounded-lg p-4 w-full shadow-sm transition-all duration-700 ease-out transform ${
                  isVisible ? "opacity-100 translate-x-0" : `opacity-0 ${fromClass}`
                }`}
                role="group"
                aria-label={`${item.event} on ${item.date}`}
              >
                <p className="text-xs uppercase tracking-widest text-muted-foreground mb-1">{item.date}</p>
                <div className="flex items-center justify-between gap-3">
                  <p className="text-lg font-heading text-charcoal">{item.event}</p>
                  <span
                    className={`timeline-status shrink-0 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] ${
